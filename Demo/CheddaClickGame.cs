...
public class CheddaClickGame : MonoBehaviour
{
                                     ...
                                     List<object> achievements, int playCount)
    {
        ...
        else if (!string.IsNullOrEmpty(localName))
        {
            ...
            if (!renameInFlight && renameRetries < 2)
            {
                renameRetries++;
                renameInFlight = true;
                Debug.Log($"[Rename] profile unnamed but localName='{localName}' - re-firing ChangeNickname");
    ...
    {
        Debug.Log($"[Rename] click. nameInput={(nameInput == null ? "NULL" : "ok")} " +
                  $"text='{(nameInput == null ? "" : nameInput.text)}' " +
                  $"playerId='{CB.GetPlayerId()}'");
        if (nameInput == null) { Debug.Log("[Rename] bail: nameInput is null"); return; }
        string wanted = nameInput.text.Trim();
        if (string.IsNullOrEmpty(wanted)) { Debug.Log("[Rename] bail: empty text"); return; }
        ...
        if (string.IsNullOrEmpty(CB.GetPlayerId()))
        {
            ...
            Debug.Log($"[Rename] no player ID yet - stashing '{wanted}' as pending");
            pendingName = wanted;
            if (nameInput != null) nameInput.text = "";
            statusText.text = $"Hi {wanted}! Your name is set after your first run.";
            return;
        }
        Debug.Log($"[Rename] calling ChangeNickname('{wanted}')");
        renameInFlight = true;
